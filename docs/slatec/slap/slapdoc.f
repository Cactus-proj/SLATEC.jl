...
C***DATE WRITTEN   880715   (YYMMDD)
C***REVISION DATE  890404   (YYMMDD)
C***CATEGORY NO.  D2B, Z
C***KEYWORDS  LIBRARY=SLATEC(SLAP),
C             TYPE=SINGLE PRECISION(SLPDOC-A),
...
C    method. 2) SSDBCG: Single precision, SLAP data structure BCG with
C    Diagonal scaling.  3)   SSLUBC: Single    precision,  BCG    with
C    incomplete  LU factorization  as   the preconditioning.  4)  SCG:
C    Single precision Conjugate  Gradient  "core" routine.  5)  SSDCG:
C    Single precision,  SLAP data  structure  Conjugate  Gradient with
C    Diagonal scaling.  6)  SSICCG:  Single   precision,    SLAP  data
...
C    
C    ============================= SCG.F =============================
C    SCG: Preconditioned Conjugate Gradient iterative Ax=b solver.
...
C    
C    ============================= SCGS.F ============================
C    SCGS: Preconditioned BiConjugate Gradient Sparse Ax=b solver.
C    SSDCGS: Diagonally Scaled CGS Sparse Ax=b Solver.
C    SSLUCS: Incomplete LU BiConjugate Gradient Sparse Ax=b solver.
C    
C    ============================ SGMRES.F ===========================
...
C    SLLTI2: Back solve routine for LDL' Factorization.
C    SSLUI: SLAP MSOLVE for LDU Factorization (set up for SSLUI2).
C    SSLUI2: SLAP Back solve for LDU Factorization.
C    SSLUTI: SLAP MTSOLV for LDU Factorization (set up for SSLUI4).
C    SSLUI4: SLAP back solve for LDU Factorization.
C    SSMMTI: SLAP MSOLVE for LDU Fact of Normal Eq (set up for SSMMI2).
C    SSMMI2: SLAP Back solve for LDU Factorization of Normal Equations.
C    
C    ========================== SLAPUTIL.F ==========================
C    SCHKW: SLAP WORK/IWORK Array Bounds Checker.
C    QS2I1R: Quick Sort Integer array coupled with int and real arrays.
